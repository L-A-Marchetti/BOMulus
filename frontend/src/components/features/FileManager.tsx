  ...
  return !WSCreator.isVisible && !WSChooser.isVisible ? (
    <div className='flex flex-col gap-8'>
      {FileManager.isVisible ? (
        ...
        <Button onClick={FileManager.toggleVisibility} text="Back" bg='bg-neutral-700' bgHover='hover:bg-neutral-900' txtColor='text-neutral-400'/>
            ...
            <Spinner text='File Manager is loading...' />
              ...
              <div className='flex flex-col gap-4'>
                {FileManager.files?.map((file) => (
                  FileManager.selectedFiles[0] === file || FileManager.selectedFiles[1] === file ? <div className='w-full h-15 border-3 border-neutral-600 bg-neutral-700 rounded-lg animate-pulse'></div> : 
                  <Files file={file}/>
                ))}
                <hr className='my-4 text-neutral-600' />
                <div className='flex gap-3'>
                { FileManager.selectedFiles[0] ?
                  <Files file={FileManager.selectedFiles[0]} /> : <div className='w-full h-15 border-3 border-neutral-600 bg-neutral-700 rounded-lg animate-pulse'></div>}
                  <div className='flex w-20 justify-center items-center'><img src={navigate_next} /></div>
                  { FileManager.selectedFiles[1] ?
                  <Files file={FileManager.selectedFiles[1]} /> : <div className='w-full h-15 border-3 border-neutral-600 bg-neutral-700 rounded-lg animate-pulse'></div>}
                  </div>
              </div>
